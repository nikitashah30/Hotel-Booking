...
const AddRoom = () => {
  ...
  return (
   <form action="">
     <Title align='left' font='outfit' title='Add Room'  subTitle='Fill in the details careflly and 
     ...
     <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
        ...
        <img src={images[key]?URL.createObjectURL(images[key]):assets.uploadArea}
         className='max-h-13 cursor-pointer opacity-80' alt="" />
     ...
     <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
        <div className='flex-1 max-w-48'>
            ...
            <select value={inputs.roomType} onChange={e=> setInputs({...inputs,roomType:e.target.value})} className='border opacity-70 border-gray-300 mt-1 rounded p-2 w-full'>
                ...
                <option value="Family suite">Famile Suite</option>
            </select>
        </div>
        <div>
     ...
     <div className='flex flex-col flex-wrap mt-1 text-gray-400 max-w-sm'>
        {Object.keys(inputs.amenities).map((amenity ,idx)=>(
            <div key={idx}>
                <input type="checkbox" id={`amenities${idx+1}`} checked={inputs.amenities[amenity]}
